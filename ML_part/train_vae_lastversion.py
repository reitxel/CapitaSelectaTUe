...
for num in range(num_preds):
    filename = "Generated_image_"+str(num)+"_"+str(epoch)+".mhd"
    array = pred[num][0]
    plt.imshow(array, cmap='gray')
    plt.show()
    itk_img = sitk.GetImageFromArray(array)
  #  itk_img.SetSpacing([0.488281, 0.488281, 1])
...
# %% view generated saved images

itk_image = sitk.ReadImage('Generated_image_1_69.mhd')
...
itk_image = sitk.ReadImage('Generated_image_4_69.mhd')
image_array2 = sitk.GetArrayViewFromImage(itk_image)
iamge_array3 = image_array-image_array2
# plot the image
plt.imshow(image_array2, cmap='gray')
plt.show()

# %% see
itk_image = sitk.ReadImage('TrainingData\p108\mr_bffe.mhd')
image_array = sitk.GetArrayViewFromImage(itk_image)
